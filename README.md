...
- **Render:** підключи репозиторій → Render підхопить `render.yaml` і задеплоїть.
...
> ⚠️ База — SQLite у `data/`. На безкоштовних хостингах файлова система
> ефемерна (скидається при передеплої). Щоб ліди зберігались — примонтуй диск
> на `/app/data` або перейди на хостинг-Postgres.
